    ...
    Fl_Group(X, Y, W, H, label)
{
	...
	mode = new Fl_Choice(X+70, Y, 100, H, "Режим:");
	mode->align(FL_ALIGN_LEFT);
	mode->add("Объекты|Линии|Сектора|Вершины");
	mode->value(0);
	...
	scale = new Fl_Choice(X+48, Y, 78, H, "Зум:");
	...
	grid_size = new Fl_Choice(X+52, Y, 72, H, "Сетка:");
	...
	status = new Fl_Box(FL_FLAT_BOX, X, Y-4, W - 4 - X, H+4, "Готово");

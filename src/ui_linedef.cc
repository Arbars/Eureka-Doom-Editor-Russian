    ...
    obj(-1), count(0)
{
	...
	which = new UI_Nombre(X+6, Y, W-12, 28, "Линия");

	Y += which->h() + 4;


	type = new UI_DynInput(X+58, Y, 75, 24, "Тип: ");
	...
	choose = new Fl_Button(X+W/2-5, Y, 80, 24, "Выбрать");
	...
	new Fl_Box(FL_NO_BOX, X+10, Y, 48, 24, "Расш.: ");
	...
	Fl_Box *flags = new Fl_Box(FL_FLAT_BOX, X+10, Y, 64, 24, "Флаги: ");
	flags->align(FL_ALIGN_INSIDE | FL_ALIGN_LEFT);


	f_automap = new Fl_Choice(X+W-118, Y, 120, 22, "Вид: ");
	f_automap->add("Нормальная|Невидимая|Наносимая|Секрет");
	f_automap->value(0);
	...
	f_upper = new Fl_Check_Button(X+28, Y+2, FW, 20, "верх. недвиж.");
	f_upper->labelsize(12);
	f_upper->callback(flags_callback, new line_flag_CB_data_c(this, MLF_UpperUnpegged));


	f_walk = new Fl_Check_Button(X+W-120, Y+2, FW, 20, "непроходимая");
	f_walk->labelsize(12);
	...
	f_lower = new Fl_Check_Button(X+28, Y+2, FW, 20, "низ недвиж.");
	f_lower->labelsize(12);
	f_lower->callback(flags_callback, new line_flag_CB_data_c(this, MLF_LowerUnpegged));


	f_mons = new Fl_Check_Button(X+W-120, Y+2, FW, 20, "блок. монст.");
	f_mons->labelsize(12);
	...
	f_passthru = new Fl_Check_Button(X+28, Y+2, FW, 20, "насквозь");
	f_passthru->labelsize(12);
	f_passthru->callback(flags_callback, new line_flag_CB_data_c(this, MLF_Boom_PassThru));
	f_passthru->hide();


	f_sound = new Fl_Check_Button(X+W-120, Y+2, FW, 20, "блок. звуки");
...
void UI_LineBox::type_callback(Fl_Widget *w, void *data)
{
	...
	if (GetCurrentObjects(&list))
	{
		...
		BA_MessageForSel("измененный тип", &list);
		BA_End();
	}
...
void UI_LineBox::SetTexture(const char *tex_name, int e_state)
{
	...
	if (GetCurrentObjects(&list))
	{
		...
		BA_MessageForSel("изменена текстура на", &list);
		BA_End();
	}
...
void UI_LineBox::CB_Paste(int new_tex)
{
	...
	BA_Message("Вставлено %s", BA_GetString(new_tex));
	BA_End();
...
void UI_LineBox::flags_callback(Fl_Widget *w, void *data)
{
	...
	if (GetCurrentObjects(&list))
	{
		...
		BA_MessageForSel("изменены флаги", &list);
		BA_End();
	}
}
...
void UI_LineBox::args_callback(Fl_Widget *w, void *data)
{
	...
	if (GetCurrentObjects(&list))
	{
		...
		BA_MessageForSel("изменены аргументы", &list);
		BA_End();
	}
}

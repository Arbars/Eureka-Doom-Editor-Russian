...
static const char * overwrite_message =
	"PWAD %s уже содержит эту карту.  "
	"Данная операция может повредить (переписать) её."
	"\n\n"
...
static bool Project_AskFile(char *filename)
{
	...
	chooser.title("Выберите файл, чтобы создать");
	...
	{
		case -1:
			LogPrintf("Новый проект: ошибка выбора файла:\n");
			LogPrintf("   %s\n", chooser.errmsg());

			DLG_Notify("Невозможно создать новый проект:\n\n%s", chooser.errmsg());
			return false;

		case 1:
			LogPrintf("Новый проект: отменено пользователем\n");
			return false;
...
void CMD_NewProject()
{
	...
	if (FileExists(filename))
	{
		...
		{
			DLG_Notify("Невозможно удалить внешний файл.");
	...
	LogPrintf("Создание нового файла : %s в %s\n", map_name, filename);
	...
	if (! wad)
	{
		DLG_Notify("Не удалось создать новый WAD файл.");
		return;
	}
...
void CMD_FreshMap()
{
	if (! edit_wad)
	{
		DLG_Notify("Невозможно обновить карту перед созданием pwad'а.");
		return;
	}

	if (edit_wad->IsReadOnly())
	{
		DLG_Notify("Невозможно обновить карту : файл только для чтения.");
		return;
	}

	if (! Main_ConfirmQuit("обновить карту"))
		return;
	...
	{
		if (DLG_Confirm("Отменить|&Переписать",
	...
	LogPrintf("Создана НОВАЯ карта : %s\n", map_name);
...
static Lump_c * Load_LookupAndSeek(const char *name)
{
	...
	if (! lump->Seek())
	{
		LogPrintf("ВНИМАНИЕ: Не удалось найти %s lump!\n", name);
	}
...
# endif
	...
	for (int i = 0 ; i < count ; i++)
	{
		...
		if (! lump->Read(&raw, sizeof(raw)))
			FatalError("Ошибка чтения вершин.\n");
...
static void LoadSectors()
{
	...
	if (! lump)
		FatalError("Нет лампов секторов!\n");
...
# endif
	...
	for (int i = 0 ; i < count ; i++)
	{
		...
		if (! lump->Read(&raw, sizeof(raw)))
			FatalError("Ошибка чтения секторов.\n");
...
{
	LogPrintf("Создание обратного сектора.\n");
...
static void LoadHeader()
{
	...
	if (! lump->Seek())
		FatalError("Ошибка поиска в заголовке лампа!\n");

	if (! lump->Read(& HeaderData[0], length))
		FatalError("Ошибка чтоения заголовка лампа.\n");
}
...
static void LoadBehavior()
{
	...
	if (! lump->Read(& BehaviorData[0], length))
		FatalError("Ошибка чтения BEHAVIOR.\n");
}
...
static void LoadThings()
{
	...
	if (! lump)
		FatalError("Нет лампов объектов!\n");
...
# endif
	...
	for (int i = 0 ; i < count ; i++)
	{
		...
		if (! lump->Read(&raw, sizeof(raw)))
			FatalError("Ошибка чтения объектов.\n");
...
# endif
	...
	for (int i = 0 ; i < count ; i++)
	{
		...
		{
			LogPrintf("ВНИМАНИЕ: стена #%d имеет битую ссылку на сектор (%d)\n",
			          i, sd->sector);
...
static void ValidateSidedefs(LineDef * ld)
{
	...
	{
		LogPrintf("ВНИМАНИЕ: линия #%d имеет битую ссылку на сторону (%d, %d)\n",
				  ld->right, ld->left);
...
# endif
	...
	for (int i = 0 ; i < count ; i++)
	{
		...
		if (! lump->Read(&raw, sizeof(raw)))
			FatalError("Ошибка чтения линий.\n");
		...
		if (ld->start >= NumVertices || ld->end >= NumVertices ||
		    ld->start == ld->end)
		{
			LogPrintf("ВНИМАНИЕ: линия #%d имеет битую ссылку на вершину (%d, %d)\n",
			          ld->start, ld->end);
...
static void RemoveUnusedVerticesAtEnd()
{
	...
	{
		LogPrintf("Удаление %d неиспользуемых вершин в конце\n", new_count);
...
{
	LogPrintf("Проблемы с загрузкой карты:\n");
	LogPrintf("   %d линий с битыми ссыками на вершины (удалены)\n", bad_linedef_count);
	LogPrintf("   %d линий с битыми ссылками на стороны\n", bad_sidedef_refs);
	LogPrintf("   %d сторон с битыми ссылками на сектора\n", bad_sector_refs);

	static char message[MSG_BUF_LEN];

	if (bad_linedef_count > 0)
	{
		sprintf(message, "Найдено %d линий с битыми ссылками на вершины.\n"
		                 "Эти линии были удалены.",
		        bad_linedef_count);
	}
	else
	{
		sprintf(message, "Найдено %d битых ссылок на сектора, %d битых ссылок на стороны.\n"
		                 "Эти ссылки были перемещены.",
		        bad_sector_refs, bad_sidedef_refs);
	}

	DLG_Notify("Отчёт валидности карты:\n\n%s", message);
}
...
void LoadLevel(Wad_file *wad, const char *level)
{
	...
	Status_Set("Загружено %s", Level_name);
...
void OpenFileMap(const char *filename, const char *map_name)
{
	...
	if (! wad)
	{
		...
		DLG_Notify("Не удалось открыть WAD-файл.");
		return;
	}
	...
	if (lev_num < 0)
	{
		DLG_Notify("В этом WAD-файле карт не найдено о_О.");
	...
	LogPrintf("Загрузка карты : %s из %s\n", map_name, edit_wad->PathName());
...
void CMD_OpenMap()
{
	...
	{
		DLG_Notify("Хммм, не могу найти эту карту !?!");
...
bool M_SaveMap()
{
	...
	if (edit_wad->IsReadOnly())
	{
		if (DLG_Confirm("Отменить|&Экспортировать",
		                "Данный pwad-файл имеет статус ТОЛЬКО ДЛЯ ЧТЕНИЯ. "
						"Хотите экспортировать карту в новый файл?") <= 0)
		{
			return false;
		}
	...
	LogPrintf("Сохраняем карту : %s в %s\n", Level_name, edit_wad->PathName());
...
bool M_ExportMap()
{
	...
	chooser.title("Выберите файл для экспорта");
	chooser.type(Fl_Native_File_Chooser::BROWSE_SAVE_FILE);
	chooser.filter("Wads\t*.wad");
	chooser.directory(Main_FileOpFolder());
	...
	{
		case -1:
			LogPrintf("Экспорт карты: ошибка выбора файла:\n");
			LogPrintf("   %s\n", chooser.errmsg());

			DLG_Notify("Не удалось экспортировать карту:\n\n%s", chooser.errmsg());
			return false;

		case 1:
			LogPrintf("Экспорт карты: отменено пользователем\n");
			return false;
	...
	if (exists)
	{
		...
		{
			DLG_Notify("Невозможно экспортировать катру в файл со статусом ТОЛЬКО ДЛЯ ЧТЕНИЯ.");
	...
	if (! wad)
	{
		DLG_Notify("Невозможно экспортировать карту:\n\n%s",
	...
	{
		if (DLG_Confirm("Отменить|&Перезаписать",
	...
	LogPrintf("Экспортируем карту : %s в %s\n", map_name, wad->PathName());
...
void CMD_CopyMap()
{
	if (! edit_wad)
	{
		DLG_Notify("Невозможно скопировать карту перед созданием PWAD.");
		return;
	}

	if (edit_wad->IsReadOnly())
	{
		DLG_Notify("Невозможно скопировать карту : файл имеет статус Только для чтения.");
		return;
	}
...
void CMD_RenameMap()
{
	if (! edit_wad)
	{
		DLG_Notify("Невозможно переименовать карту перед созданием PWAD.");
		return;
	}
...
void CMD_DeleteMap()
{
	...
	if (edit_wad->IsReadOnly())
	{
		DLG_Notify("Невозможно удалить карту : файл имеет статус Только для чтения.");
		return;
	}
	...
	if (DLG_Confirm("Отменить|&Удалить",
	                "Вы уверены, что хотите удалить эту карту? "
					"Она будет НАВСЕГДА УДАЛЕНА из текущего pwad-файла.") <= 0)
	{
		return;
	}

	LogPrintf("Удаление карты : %s...\n", Level_name);

...
void M_ValidateGivenFiles()
{
	for (int i = 0 ; i < (int)Pwad_list.size() ; i++)
	{
		if (! Wad_file::Validate(Pwad_list[i]))
			FatalError("Данный pwad не существует или повреждён: %s\n",
				Pwad_list[i]);
	}
}
...
void M_LoadRecent()
{
	...
	if (! fp)
	{
		LogPrintf("Нет списка последних файлов в: %s\n", filename);
		return;
	}

	LogPrintf("Прочтение списка последних файлов из: %s\n", filename);
...
void M_SaveRecent()
{
	...
	if (! fp)
	{
		LogPrintf("Не удалось сохранить список последних файлов в: %s\n", filename);
		return;
	}

	LogPrintf("Запись списка последних файлов в: %s\n", filename);
...
bool M_TryOpenMostRecent()
{
	...
	if (! wad)
	{
		LogPrintf("Не удалось загрузить последний pwad: %s\n", filename);
		return false;
	}
	...
	if (wad->LevelCount() == 0)
	{
		LogPrintf("Нет уровней в последнем pwad: %s\n", filename);
...
{
	LogPrintf("Анализ '%s' lump'ов\n", EUREKA_LUMP);

	Lump_c * lump = wad->FindLump(EUREKA_LUMP);

	if (! lump)
	{
		LogPrintf("--> не существует.\n");
		return true;
	}

	if (! lump->Seek())
	{
		LogPrintf("--> ошибка поиска.\n");
		return true;
	}
	...
	while (lump->GetLine(line, sizeof(line)))
	{
		...
		if (! pos || pos == line)
		{
			LogPrintf("ВНИМАНИЕ: плохой синтаксис в %s lump'ах\n", EUREKA_LUMP);
			continue;
		}
		...
		*pos++ = 0;
		...
		if (strcmp(line, "game") == 0)
		{
			...
			{
				LogPrintf("  неизвестная игра: %s\n", pos /* show full path */);

				int res = DLG_Confirm("&Игнорировать|&Отменить загрузку",
				                      "Внимание: pwad ссылается на неподдерживаемую "
									  "игру:\n\n          %s", pos);
				if (res == 1)
					return false;
			}
			else
			{
				...
				if (! new_iwad)
				{
					int res = DLG_Confirm("&Ignore|&Cancel Load",
					                      "Внимание: pwad ссылается на следующий IWAD, "
		...
		else if (strcmp(line, "resource") == 0)
		{
			...
			if (! FileExists(res))
			{
				LogPrintf("  файл не найден: %s\n", pos);

				res = FilenameReposition(pos, wad->PathName());
				LogPrintf("  пытаемся: %s\n", res);
			}
			...
			{
				DLG_Notify("Внимание: pwad ссылается на ресурсы, "
		...
		else if (strcmp(line, "port") == 0)
		{
			...
			{
				LogPrintf("  неизвестный порт: %s\n", pos);

				DLG_Notify("Внимание: pwad ссылается на неизвестный порт: \n\n%s", pos);
			}
		}
		else
		{
			LogPrintf("ВНИМАНИЕ: неизвестное ключевое слово '%s' в %s lump'е\n", line, EUREKA_LUMP);
			continue;
		}
	}
...
void M_BackupWad(Wad_file *wad)
{
	...
	{
		// Hmmm, show a dialog ??
		LogPrintf("ВНИМАНИЕ: бэкап не удался (не удалось просканировать папку)\n");
		StringFree(dir_name);
		return;
	}
	...
	{
		// Hmmm, show a dialog ??
		LogPrintf("ВНИМАНИЕ: бэкап не удался (невозможно скопировать файл)\n");
		return;
	}

	LogPrintf("Бэкап записан в: %s\n", dest_name);
}

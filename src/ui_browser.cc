	...
	kind(_kind), pic_mode(false)
{
	...
	category = new Fl_Choice(cx, cy, 200, 22, "Категория:");
	...
	search = new Fl_Input(cx, cy, 200, 22, "Поиск:");
	...
	if (strchr("O", kind))
	{
		pics = new Fl_Check_Button(X+172, cy, 64, 22, " Иконки");
		pics->value(1);
...
///  X = X + (W - MIN_BROWSER_W);
	...
	no_boom = new Fl_Box(FL_NO_BOX, X + 2, Y + 40, W - 60, 60,
						 "Это требует BOOM или совместимый с ним порт");
...
bool UI_Browser_Box::ParseUser(const char ** tokens, int num_tok)
{
	...
	if (strcmp(tokens[0], "катег") == 0 && num_tok >= 2)
	{
		CategoryByLetter(tokens[1][0]);
		return true;
	}

	if (strcmp(tokens[0], "сорт") == 0 && num_tok >= 2 && alpha)
	{
	...
	if (strcmp(tokens[0], "поиск") == 0 && num_tok >= 2)
	{
	...
	if (strcmp(tokens[0], "иконки") == 0 && num_tok >= 2 && pics)
	{
...
void UI_Browser_Box::WriteUser(FILE *fp)
{
	...
	if (isprint(cat))
		fprintf(fp, "каталог %c катег. %c\n", kind, cat);

	if (alpha)
		fprintf(fp, "каталог %c сорт. %d\n", kind, 1 - alpha->value());

	fprintf(fp, "каталог %c поиск \"%s\"\n", kind, StringTidy(search->value(), "\""));

	if (pics)
		fprintf(fp, "каталог %c иконки %d\n", kind, pics->value());
}

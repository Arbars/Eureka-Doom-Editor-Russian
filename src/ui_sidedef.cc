	...
	on_2S_line(false)
{
	...
	add_button = new Fl_Button(X + W - 120, Y, 50, 20, "ДОБ.");
	add_button->labelcolor(labelcolor());
	add_button->callback(add_callback, this);

	del_button = new Fl_Button(X + W - 65, Y, 50, 20, "УДАЛ.");
	...
	l_pic = new UI_Pic(LX, Y, 64, 64, "Ниже");
	u_pic = new UI_Pic(UX, Y, 64, 64, "Выше");
...
void UI_SideBox::UpdateLabel()
{
	if (! is_sidedef(obj))
	{
		label(is_front ? "   Нет лицевой стороны" : "   Нет обратной стороны");
		return;
	}

	char buffer[200];

	sprintf(buffer, "   %s Сторона: #%d\n",
			is_front ? "Лицевая" : "Обратная", obj);

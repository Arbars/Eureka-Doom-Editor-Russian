...
public:
	UI_PortPathDialog(const char *port_name) :
		UI_Escapable_Window(560, 250, "Настройки порта"),
		exe_name(NULL), want_close(false)
	{
		char message_buf[256];

		sprintf(message_buf, "Укажите путь к исполняемому файлу порта для %s.", port_name);
		...
		header = new Fl_Box(FL_NO_BOX, 20, 55, w() - 40, 30,
		           "Это нужно только для запуска теста карты.");
		header->align(FL_ALIGN_INSIDE | FL_ALIGN_LEFT);


		exe_display = new Fl_Output(98, 100, w()-200, 26, "Путь: ");

		Fl_Button *find_but = new Fl_Button(w()-80, 100, 60, 26, "Найти");
		...
		{
			cancel_but = new Fl_Button(w() - 260, h() - 45, 95, 30, "Отменить");
			cancel_but->callback(close_callback, this);

			ok_but = new Fl_Button(w() - 120, h() - 45, 95, 30, "ОК");
...
void CMD_TestMap()
{
	if (MadeChanges)
	{
		if (DLG_Confirm("Отменить|&Сохранить",
		                "имеются несохраненные изменения! Вы хотите их сохранить "
						"и создать ноды?") <= 0)
		{
			return;
		}

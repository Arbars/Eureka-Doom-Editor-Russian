...
void Vertex_RemoveUnused()
{
	...
	BA_Begin();
	BA_Message("удалить неиспользуемые вершины");
...
public:
	UI_Check_Vertices(bool all_mode) :
		UI_Check_base(520, 224, all_mode, "Проверить : Вершины",
				      "Результаты проверки вершин")
	{ }
...
check_result_e CHECK_Vertices(int min_severity = 0)
{
	...
	for (;;)
	{
		...
		if (sel.empty())
			dialog->AddLine("Перекрывающихся вершин нет");
		else
		{
			int approx_num = sel.count_obj() / 2;

			sprintf(check_message, "Перекрывающихся вершин: %d", approx_num);

			dialog->AddLine(check_message, 2, 210,
			                "Показать",  &UI_Check_Vertices::action_highlight,
			                "Объединить", &UI_Check_Vertices::action_merge);
		}
		...
		if (sel.empty())
			dialog->AddLine("Отдельных вершин нет");
		else
		{
			sprintf(check_message, "Отдельных вершин: %d ", sel.count_obj());

			dialog->AddLine(check_message, 2, 210,
			                "Показать",  &UI_Check_Vertices::action_show_danglers);
		}
		...
		if (sel.empty())
			dialog->AddLine("Неиспользуемых вершин нет");
		else
		{
			sprintf(check_message, "Неиспользуемых вершин: %d", sel.count_obj());

			dialog->AddLine(check_message, 1, 210,
			                "Показать",   &UI_Check_Vertices::action_show_unused,
			                "Удалить", &UI_Check_Vertices::action_remove);
		}
...
void Sectors_LogUnknown()
{
	...
	LogPrintf("\n");
	LogPrintf("Типы неизвестных секторов:\n");
	LogPrintf("{\n");
...
void SideDefs_Unpack(bool is_after_load)
{
	...
	if (false /* confirm_it */)
	{
		if (DLG_Confirm("&Не изменять|&Распаковать", unpack_confirm_message) <= 0)
			return;
	}
	...
	LogPrintf("Распаковано %d свободных сторон --> %d\n", sides.count_obj(), NumSideDefs);
}
...
check_result_e CHECK_Sectors(int min_severity = 0)
{
	...
	for (;;)
	{
		...
		if (sel.empty())
			dialog->AddLine("Нет незамкнутых секторов");
		else
		{
			sprintf(check_message, "%d незамкнутых секторов", sel.count_obj());

			dialog->AddLine(check_message, 2, 210,
			                "Показать",  &UI_Check_Sectors::action_show_unclosed,
			                "Вершины", &UI_Check_Sectors::action_show_un_verts);
		}
		...
		if (sel.empty())
			dialog->AddLine("Нет несовпадающих секторов");
		else
		{
			sprintf(check_message, "%d несовпадающих секторов", sel.count_obj());

			dialog->AddLine(check_message, 2, 210,
			                "Показать",  &UI_Check_Sectors::action_show_mismatch,
			                "Линии", &UI_Check_Sectors::action_show_mis_lines);
		}
		...
		if (sel.empty())
			dialog->AddLine("Нет секторов с потолком ниже пола");
		else
		{
			sprintf(check_message, "%d секторов с потолком ниже пола", sel.count_obj());

			dialog->AddLine(check_message, 2, 220,
			                "Показать", &UI_Check_Sectors::action_show_ceil,
			                "Исправить",  &UI_Check_Sectors::action_fix_ceil);
		}
		...
		if (sel.empty())
			dialog->AddLine("Нет shared сторон");
		else
		{
			int approx_num = sel.count_obj();

			sprintf(check_message, "%d shared свободных сторон", approx_num);

			dialog->AddLine(check_message, 1, 200,
			                "Показать",   &UI_Check_Sectors::action_show_packed,
			                "Распаковать", &UI_Check_Sectors::action_unpack);
		}
		...
		if (sel.empty())
			dialog->AddLine("Нет неиспользуемых секторов");
		else
		{
			sprintf(check_message, "%d неиспользуемых секторов", sel.count_obj());

			dialog->AddLine(check_message, 1, 170,
			                "Удалить", &UI_Check_Sectors::action_remove);
		}
		...
		if (sel.empty())
			dialog->AddLine("Нет неиспользуемых сторон");
		else
		{
			sprintf(check_message, "%d неиспользуемых сторон", sel.count_obj());

			dialog->AddLine(check_message, 1, 170,
			                "Удалить", &UI_Check_Sectors::action_remove_sidedefs);
		}
...
check_result_e CHECK_Things(int min_severity = 0)
{
	...
	for (;;)
	{
		...
		if (sel.empty())
			dialog->AddLine("Нет застрявших объектов");
		else
		{
			sprintf(check_message, "%d застрявших объектов", sel.count_obj());

			dialog->AddLine(check_message, 2, 200,
			                "Показать",  &UI_Check_Things::action_show_stuck);
		}
		...
		if (sel.empty())
			dialog->AddLine("Нет объектов вне секторов");
		else
		{
			sprintf(check_message, "%d объектов вне секторов", sel.count_obj());

			dialog->AddLine(check_message, 1, 200,
			                "Показать",   &UI_Check_Things::action_show_void,
			                "Удалить", &UI_Check_Things::action_remove_void);
		}
		...
		if (game_info.no_need_players)
			...
			dialog->AddLine("Не найдена точка старта для Игрока 1!", 2);
		else if (! (mask & 2))
			dialog->AddLine("Не найдена точка старта для Игрока 2!", 1);
		else if (! (mask & 4))
			dialog->AddLine("Не найдена точка старта для Игрока 3!", 1);
		else if (! (mask & 8))
			dialog->AddLine("Не найдена точка старта для Игрока 4!", 1);
		else
			dialog->AddLine("Найдены точки старта для всех 4 игроков!");
		...
		{
			dialog->AddLine("Отсутствуют точки старта игроков для DeathMatch", 1);
		}
		else if (dm_num < game_info.min_dm_starts)
		{
			sprintf(check_message, "Найдено %d точек старта для DeathMatch -- нужно как минимум %d", dm_num,
			        game_info.min_dm_starts);
			dialog->AddLine(check_message, 1);
		}
		else if (dm_num > game_info.max_dm_starts)
		{
			sprintf(check_message, "Найдено %d точек старта для DeathMatch -- максимальное число %d", dm_num,
			        game_info.max_dm_starts);
			dialog->AddLine(check_message, 2);
		}
		else
		{
			sprintf(check_message, "Найдено %d точек старта для DeathMatch -- OK", dm_num);
...
void LineDefs_LogUnknown()
{
	...
	LogPrintf("\n");
	LogPrintf("Линий неизвестного типа:\n");
	LogPrintf("{\n");
...
check_result_e CHECK_LineDefs(int min_severity)
{
	...
	for (;;)
	{
		...
		if (sel.empty())
			dialog->AddLine("Нет линий с нулевой длинной");
		else
		{
			sprintf(check_buffer, "%d линий с нулевой длинной", sel.count_obj());

			dialog->AddLine(check_buffer, 2, 220,
			                "Показать",   &UI_Check_LineDefs::action_show_zero,
			                "Удалить", &UI_Check_LineDefs::action_remove_zero);
		}
		...
		if (sel.empty())
			dialog->AddLine("Нет перекрывающихся линий");
		else
		{
			sprintf(check_buffer, "%d перекрывающихся линий", sel.count_obj());

			dialog->AddLine(check_buffer, 2, 220,
			                "Показать",   &UI_Check_LineDefs::action_show_overlap,
			                "Удалить", &UI_Check_LineDefs::action_remove_overlap);
		}
		...
		if (sel.empty())
			dialog->AddLine("Нет пересекающихся линий");
		else
		{
			sprintf(check_buffer, "%d пересекающихся линий", sel.count_obj());

			dialog->AddLine(check_buffer, 2, 220,
			                "Показать", &UI_Check_LineDefs::action_show_crossing);
		}
		...
		if (sel.empty())
			dialog->AddLine("Нет линий без правой стороны");
		else
		{
			sprintf(check_buffer, "%d линий без правой стороны", sel.count_obj());

			dialog->AddLine(check_buffer, 2, 250,
			                "Показать", &UI_Check_LineDefs::action_show_mis_right);
		}
		...
		if (sel.empty())
			dialog->AddLine("Нет \"ручных\" дверей на односторонних линиях");
		else
		{
			sprintf(check_buffer, "%d \"ручных\" дверей на односторонних линиях", sel.count_obj());

			dialog->AddLine(check_buffer, 2, 250,
			                "Показать", &UI_Check_LineDefs::action_show_manual_doors,
			                "Исправить",  &UI_Check_LineDefs::action_fix_manual_doors);
		}
		...
		if (sel.empty())
			dialog->AddLine("Нет неблокирующих односторонних линий");
		else
		{
			sprintf(check_buffer, "%d неблокирующих односторонних линий", sel.count_obj());

			dialog->AddLine(check_buffer, 1, 300,
			                "Показать", &UI_Check_LineDefs::action_show_lack_impass,
			                "Исправить",  &UI_Check_LineDefs::action_fix_lack_impass);
		}
		...
		if (sel.empty())
			dialog->AddLine("Нет линий с неверным двусторонним флагом");
		else
		{
			sprintf(check_buffer, "%d линий с неверным двусторонним флагом", sel.count_obj());

			dialog->AddLine(check_buffer, 1, 285,
			                "Показать", &UI_Check_LineDefs::action_show_bad_2s_flag,
			                "Исправить",  &UI_Check_LineDefs::action_fix_bad_2s_flag);
		}
...
public:
	UI_Check_Tags(bool all_mode) :
		UI_Check_base(520, 286, all_mode, "Проверка тегов", "Результаты проверки тегов"),
		fresh_tag(0)
	{ }
...
check_result_e CHECK_Tags(int min_severity)
{
	...
	for (;;)
	{
		...
		if (max_tag <= 0)
			dialog->AddLine("Нет используемых тегов");
		else
		{
			sprintf(check_buffer, "Наименьший тег: %d   Наибольший тег: %d", min_tag, max_tag);
		    ...
		    edit.Selected->notempty())
		{
			...
			dialog->AddGap(10);
			dialog->AddLine("Применить свежий тег к выделенному :", 0, 215, "Применить",
...
void Textures_LogTransparent()
{
	...
	LogPrintf("\n");
	LogPrintf("Прозрачные текстуры на сплошных стенах:\n");
	LogPrintf("{\n");
...
void Textures_LogMedusa()
{
	...
	LogPrintf("\n");
	LogPrintf("Текстуры с эффектом Медузы:\n");
	LogPrintf("{\n");
...
void Textures_LogUnknown(bool do_flat)
{
	...
	LogPrintf("\n");
	LogPrintf("Неизвестные %s:\n", do_flat ? "Плоскости" : "Текстуры");
	LogPrintf("{\n");
...
public:
	UI_Check_Textures(bool all_mode) :
		UI_Check_base(565, 286, all_mode, "Проверить : Текстуры",
...
check_result_e CHECK_Textures(int min_severity)
{
	...
	for (;;)
	{
		...
		if (sel.empty())
			dialog->AddLine("Нет неизвестных текстур");
		else
		{
			sprintf(check_buffer, "%d неизвестных текстур", (int)names.size());
		...
		if (sel.empty())
			dialog->AddLine("Неизвестных плоскостей нет");
		else
		{
			sprintf(check_buffer, "Неизвестных плоскостей: %d", (int)names.size());
		...
		if (sel.empty())
			dialog->AddLine("Отсутствующих текстур на стенах нет.");
		else
		{
			sprintf(check_buffer, "На стенах текстур отсутствует %d ", sel.count_obj());

			dialog->AddLine(check_buffer, 1, 270,
			                "Показать", &UI_Check_Textures::action_show_missing,
			                "Исправить",  &UI_Check_Textures::action_fix_missing);
		}
		...
		if (sel.empty())
			dialog->AddLine("Нет прозрачных текстур на сплошных стенах");
		else
		{
			sprintf(check_buffer, "%d прозрачных текстур на сплошных стенах", sel.count_obj());
...
void CHECK_All(bool major_stuff)
{
	...
	if (no_worries)
	{
		DLG_Notify(major_stuff ? "Нет крупных проблем." :

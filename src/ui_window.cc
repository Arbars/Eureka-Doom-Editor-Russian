	...
	last_x(0), last_y(0), last_w(0), last_h(0)
{
	...
	op_thing  = new Fl_Menu_Button(0, 0, 99, 99, "Действия в объектами");
	op_line   = new Fl_Menu_Button(0, 0, 99, 99, "Действия с линиями");
	op_sector = new Fl_Menu_Button(0, 0, 99, 99, "Действия с секторами");
	op_vertex = new Fl_Menu_Button(0, 0, 99, 99, "Действия с объектами");
	op_render = new Fl_Menu_Button(0, 0, 99, 99, "Действия с 3D");
...
UI_LogViewer::UI_LogViewer() :
	UI_Escapable_Window(580, 400, "Просмотр логов Eureka")
{
	box(FL_NO_BOX);
...

	{
		...
		int bx2 = bx;
		{
			Fl_Button * but = new Fl_Button(bx, ey + 15, 100, 35, "Закрыть");
			but->labelfont(1);
		...
		bx = 30;
		{
			Fl_Button * but = new Fl_Button(bx, ey + 15, 100, 35, "Сохранить...");
			but->callback(save_callback, this);
		}

		bx += 105;
		{
			Fl_Button * but = new Fl_Button(bx, ey + 15, 100, 35, "Очистить");
			but->callback(clear_callback, this);
		}

		bx += 105;
		{
			copy_but = new Fl_Button(bx, ey + 15, 100, 35, "Копировать");
...
void UI_LogViewer::save_callback(Fl_Widget *w, void *data)
{
	...
	chooser.title("Выберите файл, чтобы сохранить");
	...
	{
		case -1:
			DLG_Notify("Не удалось сохранить файл:\n\n%s",
					   chooser.errmsg());
			return;
	...
	if (! fp)
	{
		...
		DLG_Notify("Не удалось сохранить log файл:\n\n%s", filename);
		return;
	}

...
void DLG_ShowError(const char *msg, ...)
{
	...
	DialogShowAndRun('!', dialog_buffer, "Eureka - Ошибка", link_title, link_url);
}
...
void DLG_Notify(const char *msg, ...)
{
	...
	DialogShowAndRun('i', dialog_buffer, "Eureka - Напоминание");
}
...
int DLG_Confirm(const char *buttons, const char *msg, ...)
{
	...
	return DialogShowAndRun('?', dialog_buffer, "Eureka - Подтверждение",
							NULL, NULL, &labels);
}

    ...
    obj(-1), count(0)
{
	...
	which = new UI_Nombre(X+6, Y, W-12, 28, "Объект");
	...
	sprite = new UI_Pic(X + W - 120, Y + 10, 100,100, "Спрайт");
	...
	angle = new Fl_Int_Input(X+70, Y, 64, 24, "Угол: ");
	...
	Fl_Box *opt_lab = new Fl_Box(X+10, Y, W, 22, "Флаги опций:");
	...
	spec_choose = new Fl_Button(X+W/2+24, Y, 80, 24, "Выбрать");
	spec_choose->callback(button_callback, this);
	spec_choose->hide();

	Y = Y + spec_type->h() + 2;

	spec_desc = new Fl_Output(X+74, Y, W-86, 24, "Расш.: ");

...
UI_ChooseMap::UI_ChooseMap(const char *initial_name,
	...
	UI_Escapable_Window(435, 385, "Выберите Карту"),
	...
	action(ACT_none)
{
	...
	{
		map_name = new Fl_Input(150, 35, 120, 25, "Слот Карты: ");
...

	{
		...
		Fl_Button *cancel = new Fl_Button(130, bottom_y + 17, 95, 35, "Отменить");
...
UI_OpenMap::UI_OpenMap() :
	UI_Escapable_Window(420, 470, "Открыть Карту"),
	action(ACT_none),
	loaded_wad(NULL),
	 using_wad(NULL)
{
	...
	{
		look_where = new Fl_Choice(150, 80, 190, 25, "Найти карту в:  ");
		look_where->labelfont(FL_HELVETICA_BOLD);
		look_where->add("самом PWAD|IWAD игры|ресурсном WAD");
	...
	{
		Fl_Box* o = new Fl_Box(15, 15, 270, 20, "Файл PWAD:");
	...
	Fl_Button *load_but = new Fl_Button(330, 39, 85, 28, "Загрузить");
	load_but->callback(load_callback, this);


	map_name = new Fl_Input(125, 125, 100, 26, "Слот карты: ");
	map_name->labelfont(FL_HELVETICA_BOLD);
	map_name->when(FL_WHEN_CHANGED);
	map_name->callback(input_callback, this);

	{
		Fl_Box *o = new Fl_Box(230, 125, 180, 26, "Доступные карты:");
		// o->labelfont(FL_HELVETICA_BOLD);
		o->align(FL_ALIGN_LEFT | FL_ALIGN_INSIDE);
	}
...

	{
		...
		Fl_Button * cancel = new Fl_Button(100, bottom_y + 20, 95, 35, "Отменить");
		cancel->callback(close_callback, this);
...
void UI_OpenMap::LoadFile()
{
	...
	chooser.title("Выберите файл для открытия");
	...
	{
		case -1:
			LogPrintf("Открыть карту: Ошибка выбора файла:\n");
			LogPrintf("   %s\n", chooser.errmsg());

			DLG_Notify("Не удалось открыть карту:\n\n%s",
					   chooser.errmsg());
			return;

		case 1:
			LogPrintf("Открыть карту: отменено пользователем\n");
			return;
	...
	if (! wad)
	{
		...
		DLG_Notify("Невозможно открыть выбранный файл.\n\n"
				   "Попробуйте ещё раз.");
		return;
	}
	...
	if (wad->LevelCount() < 0)
	{
		DLG_Notify("WAD не содержит карт.\n\n"
				   "Попробуйте ещё раз.");
		return;
	}
...
#define STARTUP_MSG  "Не удалось найти IWAD."


UI_ProjectSetup::UI_ProjectSetup(bool new_project, bool is_startup) :
	UI_Escapable_Window(400, is_startup ? 200 : 400, new_project ? "Новый проект" : "Настройки проекта"),
	action(ACT_none),
	game(NULL), port(NULL), map_format(MAPF_INVALID)
{
	...
	if (is_startup)
	{
		Fl_Box * message = new Fl_Box(FL_FLAT_BOX, 15, 15, 400, 46, STARTUP_MSG);
	...
	game_choice = new Fl_Choice(140, by+25, 150, 29, "IWAD игры: ");
	...
	{
		Fl_Button* o = new Fl_Button(300, by+27, 85, 25, "Найти");
		o->callback((Fl_Callback*)find_callback, this);
	}

	port_choice = new Fl_Choice(140, by+60, 150, 29, "Порт: ");
	...
	{
		Fl_Button* o = new Fl_Button(300, by+60, 85, 25, "Настроить");
		o->callback((Fl_Callback*)setup_callback, this);
	}

	format_choice = new Fl_Choice(140, by+95, 150, 29, "Тип карты: ");
	...
	if (! is_startup)
	{
		Fl_Box *res_title = new Fl_Box(15, by+135, 185, 35, "Файл ресурсов:");
	...
	for (int r = 0 ; r < RES_NUM ; r++)
	{
		...
		res_name[r] = new Fl_Output(55, cy, 190, 25);
		res_name[r]->copy_label(res_label);

		Fl_Button *kill = new Fl_Button(260, cy, 30, 25, "x");
		kill->labelsize(20);
		kill->callback((Fl_Callback*)kill_callback, (void *)(long)r);

		Fl_Button *load = new Fl_Button(300, cy, 85, 25, "Загрузить");
	...
	// bottom buttons
	{
		...
		const char *cancel_text = is_startup ? "Выйти" : "Отменить";

		cancel = new Fl_Button(90, g->y() + 14, 110, 35, cancel_text);
		cancel->callback((Fl_Callback*)close_callback, this);

		const char *ok_text = (is_startup | new_project) ? "OK" : "Применить";

		ok_but = new Fl_Button(240, g->y() + 14, 110, 35, ok_text);
		ok_but->labelfont(FL_HELVETICA_BOLD);
...
void UI_ProjectSetup::PopulateMapFormat()
{
	...
	{
		strcat(menu_string, "Формат Doom");
		entry_id++;
	}
	...
	if (usable_formats & (1 << MAPF_Hexen))
	{
		...
		strcat(menu_string, "Формат Hexen");
		entry_id++;
	}
...
void UI_ProjectSetup::find_callback(Fl_Button *w, void *data)
{
	...
	{
		case -1:  // error
			DLG_Notify("Не удалось открыть этот wad:\n\n%s", chooser.errmsg());
			return;
	...
	{
		DLG_Notify("Эта игра не поддерживается (нет файла описания).\n\n"
		           "Попробуйте ещё раз.");
		return;
	}
...
void UI_ProjectSetup::load_callback(Fl_Button *w, void *data)
{
	...
	{
		case -1:  // error
			DLG_Notify("Невозможно открыть этот wad:\n\n%s", chooser.errmsg());
			return;
